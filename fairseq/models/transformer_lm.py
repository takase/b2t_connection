...
class TransformerLanguageModel(FairseqLanguageModel):
    ...
    def add_args(parser):
        ...
        #args for modify residual (and layer normalization)
        parser.add_argument('--residual-bottom-to-top', default=False, action='store_true',
                            help='Residual connection from bottom to top in each layer')
        parser.add_argument('--residual-max-updates', default=-1, type=int,
                            help='The number of maximum updates of residual connection from bottom to top, -1 means use residual every time')
        parser.add_argument('--no-layer-norm', default=False, action='store_true',
                            help='Remove layer normalization')
        parser.add_argument('--no-inner-layer-norm', default=False, action='store_true',
                            help='Remove inner layer normalization')
        parser.add_argument('--decay-inner-residual', default=False, action='store_true',
                            help='Decay inner residual connection based on residual max updates')
        # fmt: on
...
def base_lm_architecture(args):
    ...
    #to solve error in language modeling
    args.sampling_method = getattr(args, 'sampling_method', None)
    args.dec_replace_rate = getattr(args, 'dec_replace_rate', 0)
    args.decay_val = getattr(args, 'decay_val', 0)
    args.residual_max_updates = getattr(args, 'residual_max_updates', -1)
    args.ordinary_sinpos = getattr(args, 'ordinary_sinpos', True)
    args.represent_length_by_lrpe = getattr(args, 'represent_length_by_lrpe', False)
    args.represent_length_by_ldpe = getattr(args, 'represent_length_by_ldpe', False)
    args.residual_bottom_to_top = getattr(args, 'residual_bottom_to_top', False)
    args.no_layer_norm = getattr(args, 'no_layer_norm', False)
    args.no_inner_layer_norm = getattr(args, 'no_inner_layer_norm', False)
    args.decay_inner_residual = getattr(args, 'decay_inner_residual', -1)
    args.stop_relu_dropout_update = getattr(args, 'stop_relu_dropout_update', 0)
