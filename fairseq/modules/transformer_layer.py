...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        self.residual_bottom_to_top = args.residual_bottom_to_top
        self.no_layer_norm = args.no_layer_norm
        if self.no_layer_norm:
    ...
    def forward(self, x, encoder_padding_mask, attn_mask: Optional[Tensor] = None):
        ...
        if self.residual_bottom_to_top:
            residual_bottom = x
        if self.normalize_before and self.self_attn_layer_norm is not None:
        ...
        x = residual + x
        if not self.normalize_before and self.self_attn_layer_norm is not None:
            x = self.self_attn_layer_norm(x)
        ...
        if self.residual_bottom_to_top:
            if self.no_layer_norm:
                x = self.scale4residual * residual_bottom + self.scale4output * x
            else:
                x = residual_bottom + x
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(
        ...
        if self.encoder_attn is not None:
            residual = x
            if self.normalize_before and self.encoder_attn_layer_norm is not None:
            ...
            x = F.dropout(x, p=self.dropout, training=self.training)
            x = residual + x
            if not self.normalize_before and self.encoder_attn_layer_norm is not None:
                x = self.encoder_attn_layer_norm(x)
